...
func SetupAPIRoutes(r *gin.Engine, cfg *config.Config) {
	...
	friend := apiV1.Group("/friend")
	{
		...
		// 好友申请相关的路由
		friend.POST("/request", friendHandler.SendFriendRequest)
		friend.GET("/request/received", friendHandler.GetReceivedRequests)
		friend.GET("/request/sent", friendHandler.GetSentRequests)
		friend.POST("/request/:id/accept", friendHandler.AcceptFriendRequest)
		friend.POST("/request/:id/reject", friendHandler.RejectFriendRequest)
		friend.DELETE("/request/:id", friendHandler.CancelFriendRequest)
		friend.GET("/request/pending-count", friendHandler.GetPendingRequestCount)
	}
