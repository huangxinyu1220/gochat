...
// GetPrivateMessagesWithUserInfo 获取单聊历史消息（包含用户信息）
func (s *MessageService) GetPrivateMessagesWithUserInfo(userID1, userID2 int64, page, pageSize int) ([]MessageInfo, int64, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			log.Printf("Error scanning private message row: %v", err)
			return nil, 0, err
		}
...
// GetGroupMessagesWithUserInfo 获取群聊历史消息（包含用户信息）
func (s *MessageService) GetGroupMessagesWithUserInfo(groupID int64, page, pageSize int) ([]MessageInfo, int64, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			log.Printf("Error scanning group message row: %v", err)
			return nil, 0, err
		}
