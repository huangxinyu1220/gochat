...
const MainPage = () => {
  ...
  useEffect(() => {
    if (friendSearchValue.trim()) {
      ...
      const filtered = friends.filter(friend =>
        (friend.nickname && friend.nickname.toLowerCase().includes(friendSearchValue.toLowerCase())) ||
        (friend.phone && friend.phone.toLowerCase().includes(friendSearchValue.toLowerCase()))
      );
  ...
  const handleAddFriend = async (user) => {
    try {
      // 标记正在处理
      setSearchResults(prev => prev.map(item =>
        item.id === user.id ? { ...item, processing: true } : item
      ));

      await friendAPI.addFriend(user.id);
      message.success(`已发送好友请求给 ${user.nickname}`);
      ...
      // 恢复处理状态
      setSearchResults(prev => prev.map(item =>
        item.id === user.id ? { ...item, processing: false } : item
      ));
    }
  };
    ...
    if (activeNav === 'friends') {
      ...
      const groupFriendsByInitial = (friendList) => {
        ...
        friendList.forEach(friend => {
          const firstChar = (friend.nickname || '')[0]?.toUpperCase();
        ...
        [...'ABCDEFGHIJKLMNOPQRSTUVWXYZ', '#'].forEach(letter => {
          if (groups[letter] && groups[letter].length > 0) {
            // 组内按名称排序
            groups[letter].sort((a, b) => (a.nickname || '').localeCompare(b.nickname || ''));
